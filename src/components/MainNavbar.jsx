...
export default function MainNavbar({ currentPath = '' }) {
    ...
    const navItems = [
        ...
        },
        {
            ...
            dropdownItems: [
                { label: 'Generación', path: '/horarios', icon: <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M13 10V3L4 14h7v7l9-11h-7z" /></svg> },
        ...
        <header className="fixed top-0 left-0 w-full bg-white/95 backdrop-blur-sm z-50 flex items-center justify-between px-6 h-20 transition-all duration-300">
                ...
                {navItems.map((item) => {
                                        ...
                                        return item.dropdownItems.map((child, idx) => {
                                            ...
                                            return (
                                                <a
                                                    ...
                                                    className={`relative group flex items-center gap-2 px-4 py-2 rounded-full transition-all duration-300 text-[13px] font-bold ${child.locked
                    ...
                    return item.path ? (
                        <a
                            key={item.id}
                            href={item.path}
                            className={`flex items-center justify-center gap-2 px-5 py-2.5 rounded-full transition-all duration-300 font-bold text-[14px] cursor-pointer ${isActive
            ...
            <div className="relative flex items-center">
                <div className="group relative">
                    <div className="w-10 h-10 rounded-full bg-[var(--color-brand-light)]/20 flex items-center justify-center text-[var(--color-brand-primary)] font-bold border border-[var(--color-brand-primary)]/20 cursor-pointer hover:bg-[var(--color-brand-primary)] hover:text-white transition-colors">
                        U
                    </div>
                    {/* Dropdown Menu (Hover) */}
                    <div className="absolute right-0 top-full pt-3 w-48 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 transform translate-y-2 group-hover:translate-y-0 z-50">
                        <div className="bg-white rounded-2xl shadow-[0_10px_40px_rgba(0,0,0,0.1)] border border-slate-100 p-2">
                            <div className="px-3 py-2 border-b border-slate-50 mb-2">
                                <p className="text-sm font-black text-slate-800">Administrador</p>
                                <p className="text-[11px] text-slate-500 font-medium">Cuenta Activa</p>
                            </div>
                            <button
                                onClick={handleLogout}
                                className="w-full flex items-center gap-2 px-3 py-2.5 rounded-xl text-[13px] font-bold text-slate-600 hover:text-red-600 hover:bg-red-50 transition-all cursor-pointer"
                            >
